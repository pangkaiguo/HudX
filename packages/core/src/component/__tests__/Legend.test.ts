...
describe('Legend', () => {
  ...
  it('should support itemMaxWidth', () => {
    ...
    legend.setItems([{ name: 'Very Long Series Name That Should Wrap', color: 'red' }]);
  ...
  it('should support align option', () => {
    ...
    const children = legend.children();
    const rects = children.filter(c => c instanceof Rect && c.style.fill === 'transparent');
  ...
  it('should pass data to formatter in HTML mode', () => {
    ...
    legend.setDomContainer(container);
    legend.setItems([
      { name: 'A', color: 'red', data: { value: 100 } }
    ]);
  ...
  it('should wrap items vertically (multi-column) when height is exceeded', () => {
    ...
    const children = legend.children();
    const rects = children.filter(c => c instanceof Rect && c.style.fill === 'red');
