...
const ChartCard = ({ example, onClick }: { example: Example; onClick: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 16px' }}>
        <div style={{ fontWeight: 'bold', marginBottom: 4, fontSize: 14 }}>{example.title}</div>
...
const Dashboard = ({ activeCategory, onSelectExample, theme, onThemeChange }: { activeCategory: string, onSelectExample: (id: string) => void, theme: Theme, onThemeChange: (theme: Theme) => void }) => {
  ...
  return (
    <div style={{ flex: 1, height: '100%', overflowY: 'auto', backgroundColor: '#f5f7fa' }}>
      <div style={{ padding: '20px 40px', backgroundColor: '#fff', borderBottom: '1px solid #e0e0e0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ margin: 0, fontSize: 24, fontWeight: 'normal' }}>{categoryLabel}</h2>
      ...
      <div style={{ padding: 40 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
          {filteredExamples.map(ex => (
            <ChartCard key={ex.id} example={ex} onClick={() => onSelectExample(ex.id)} />
          ))}
          {filteredExamples.length === 0 && (
            <div style={{ color: '#999' }}>No examples yet for this category.</div>
          )}
        </div>
      </div>
    </div>
...
const OldApp = memo(function OldApp({ theme, setTheme, setMode }: { theme: 'Light' | 'Dark', setTheme: (t: 'Light' | 'Dark') => void, setMode: (m: 'classic' | 'modern') => void }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: theme === 'Dark' ? '#1a1a1a' : '#fff' }}>
      ...
      >
        ...
        <div style={{ marginBottom: 20, padding: '10px 0', borderBottom: `1px solid ${theme === 'Dark' ? '#333' : '#e0e0e0'}`, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 14, color: theme === 'Dark' ? '#eee' : '#333' }}>
            <span>UI Mode:</span>
            <div style={{ display: 'flex', border: `1px solid ${theme === 'Dark' ? '#333' : '#e0e0e0'}`, borderRadius: 4, overflow: 'hidden' }}>
              <button
            ...
            <div style={{ display: 'flex', border: `1px solid ${theme === 'Dark' ? '#333' : '#e0e0e0'}`, borderRadius: 4, overflow: 'hidden' }}>
              <button
                ...
                style={{
                  padding: '4px 12px',
                  background: theme === 'Light' ? '#4096ff' : 'transparent',
                  color: theme === 'Light' ? '#fff' : (theme === 'Dark' ? '#fff' : '#333'),
                  border: 'none',
                  cursor: 'pointer',
                  fontSize: 12
                }}
...
const ModernApp = ({ theme, setTheme, setMode }: { theme: Theme, setTheme: (t: Theme) => void, setMode: (m: 'classic' | 'modern') => void }) => {
  const [activeCategory, setActiveCategory] = useState('line');
  const [activeExampleId, setActiveExampleId] = useState<string | null>(null);

  const activeExample = useMemo(() => examples.find(e => e.id === activeExampleId), [activeExampleId]);
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        <div style={{ padding: '10px 20px', backgroundColor: '#fff', borderBottom: '1px solid #e0e0e0', display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
          <div style={{ display: 'flex', border: '1px solid #e0e0e0', borderRadius: 4, overflow: 'hidden' }}>
            <button
              onClick={() => setMode('classic')}
              style={{
...
export default function App() {
  ...
  const normalizedTheme = typeof theme === 'string' ? (theme.toLowerCase() === 'dark' ? 'Dark' : 'Light') : 'Light';
  const setNormalizedTheme = (t: 'Light' | 'Dark') => setTheme(t.toLowerCase() as Theme);

  return mode === 'classic' ? (
    <OldApp theme={normalizedTheme} setTheme={setNormalizedTheme} setMode={setMode} />
  ) : (
