...
export const Codebox: React.FC<CodeboxProps> = ({ initialCode, onBack, title, theme, onThemeChange }) => {
  ...
  const runCode = useCallback(() => {
    try {
      ...
      if (result) {
        ...
        setError('Code did not return an "option" object or assign to "option".');
      }
    } catch (err: any) {
      console.error(err);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: bgColor, color: textColor }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <button onClick={onBack} style={{ cursor: 'pointer', background: 'none', border: 'none', fontSize: 16, color: textColor }}>
            ← Back
          </button>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        {/* Left Pane: Editor */}
        <div style={{ width: '40%', display: 'flex', flexDirection: 'column', borderRight: `1px solid ${borderColor}` }}>
          <div style={{
            height: 40,
            borderBottom: `1px solid ${borderColor}`,
            display: 'flex',
            ...
            <button
              style={{
                padding: '0 15px',
                height: '100%',
                background: activeTab === 'JS' ? (theme === 'dark' ? '#1e1e1e' : '#fff') : 'transparent',
                border: 'none',
                color: textColor,
                cursor: 'pointer',
            ...
            <button
              style={{
                padding: '0 15px',
                height: '100%',
                background: activeTab === 'TS' ? (theme === 'dark' ? '#1e1e1e' : '#fff') : 'transparent',
                border: 'none',
                color: textColor,
                cursor: 'pointer',
          ...
          />
          {error && (
            <div style={{ padding: 10, color: 'red', fontSize: 12, borderTop: `1px solid ${borderColor}` }}>
              {error}
            </div>
          )}
        </div>
        ...
        {/* Right Pane: Preview */}
        <div style={{ flex: 1, padding: 20, display: 'flex', flexDirection: 'column', backgroundColor: theme === 'dark' ? '#100c2a' : '#fff' }}>
          <div ref={chartContainerRef} style={{ flex: 1, minHeight: 0, border: `1px solid ${borderColor}`, borderRadius: 8, overflow: 'hidden' }}>
            <HChart
              option={option}
              theme={theme}
          ...
          <div style={{ height: 40, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, marginTop: 10 }}>
            <button onClick={handleDownload} style={{ padding: '5px 10px', background: 'none', border: `1px solid ${borderColor}`, borderRadius: 4, color: textColor, fontSize: 12, cursor: 'pointer' }}>Download Code</button>
            <div style={{ display: 'flex', border: `1px solid ${borderColor}`, borderRadius: 4, overflow: 'hidden' }}>
              <button onClick={() => handleScreenshot('png')} style={{ padding: '5px 10px', background: 'none', borderRight: `1px solid ${borderColor}`, borderTop: 'none', borderBottom: 'none', borderLeft: 'none', color: textColor, fontSize: 12, cursor: 'pointer' }}>PNG</button>
              <button onClick={() => handleScreenshot('jpeg')} style={{ padding: '5px 10px', background: 'none', borderRight: `1px solid ${borderColor}`, borderTop: 'none', borderBottom: 'none', borderLeft: 'none', color: textColor, fontSize: 12, cursor: 'pointer' }}>JPG</button>
              <button onClick={() => handleScreenshot('svg')} style={{ padding: '5px 10px', background: 'none', border: 'none', color: textColor, fontSize: 12, cursor: 'pointer', opacity: renderMode === 'canvas' ? 0.5 : 1, pointerEvents: renderMode === 'canvas' ? 'none' : 'auto' }}>SVG</button>
            </div>
          </div>
        </div>
      </div>
